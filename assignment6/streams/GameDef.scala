...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = 
       List((this.left, Left),(this.right, Right),(this.up, Down),(this.down, Down))
    ...
    def legalNeighbors: List[(Block, Move)] = for {
      choice <- this.neighbors
      if (choice._1.isLegal)
    } yield (choice)
     ...
     */
    def isLegal: Boolean = 
      if (((b1.x >= 0) && (b1.y >= 0) && (b2.x >= 0) && (b2.y >= 0)) && 
          ((terrain(b1)) && (terrain(b2)))) true else false
